        ...
        if receive_decode == "*":
            ...
            if data_list:
                ...
                if address == "016900": #start time
                    ...
                    calibration = struct.unpack('!f', 
                        bytes.fromhex(value[4:8] + value[0:4]))[0]
                    calibration75 = struct.unpack('!f',
                        bytes.fromhex(value[12:16] + value[8:12]))[0]
                    result = struct.unpack('!f',
                        bytes.fromhex(value[20:24] + value[16:20]))[0]
                        
                    #server_log([tester, product,start, end, max_load, status])
                    dir_log([tester, product, start, end, calibration, calibration75, result, status])
